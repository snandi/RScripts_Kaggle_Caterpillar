...
train$Year <- format(as.yearqtr(train$quote_date), '%Y')
for( i in 1:length(train$Year) ){
  if(! train$Year[i]%in% c("2011","2012","2013","2014")){
    train$Year[i]="other"
  }
}
train$Year <- as.factor(train$Year)
#table(train$Year)
train$Qtr <- format(as.yearqtr(train$quote_date), '%q')
train$Qtr <- as.factor(train$Qtr)
...
write.csv(submitDb, "submit1.csv", row.names = FALSE, quote = FALSE)
